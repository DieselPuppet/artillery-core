  ...
  return function scenario(initialContext, callback) {
    function zero(callback) {
      ...
      let options = _.extend(tls, config.ws);
       ...
       ee.emit('started');
       if (config.scriptTarget) {
           debug("Eval script target");
           let processFunc = config.processor[config.scriptTarget];
           debug(processFunc(config.target, function(wsURL) {
              debug("Opening ws " +wsURL);
                    let ws = new WebSocket(wsURL, options);
                    ws.on('open', function() {
                      initialContext.ws = ws;
                      return callback(null, initialContext);
                    });
                    if (config.receiveHandler){
                        ws.on('message', config.processor[config.receiveHandler]);
                    }
